...
//       実際にキャンバスが表示される大きさ
//
//   (例）canvas.style.width  = 400px canvas.width  = 200px の場合は、横に2倍して表示される
//   (例）canvas.style.height = 150px canvas.height = 300px の場合は、縦に1/2倍して表示される
...
function TMamHandwritten(drawCanvasId, clearButtonId){
  ...
  window.addEventListener("DOMContentLoaded", function(){
    ...
    // canvas.width と canvas.style.width の比率を取得する
    this.rate.x = parseInt(this.can.width) / parseInt(s.width);
    
	// canvas.height と canvas.style.heightの比率を取得する
  ...
  // 手書き画像を PNG形式で Data URI スキーム (base64) を使って文字列に変換して POST する場合
